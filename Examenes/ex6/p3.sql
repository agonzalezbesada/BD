...
SELECT ProductName, Price FROM Products WHERE (SELECT MIN(Price) FROM Products) ORDER BY Price LIMIT 1;
-- ¿Cual es el producto cuyo precio sea al menos 10 veces el pedido mínimo (quantity) de los pedidos (OrderDetails)?
SELECT * FROM Products WHERE Price >= (SELECT (MIN(Quantity) * 10) FROM OrderDetails);
...
SELECT * FROM Products WHERE Price > (SELECT MAX(Price) FROM Products WHERE ProductID IN(3, 6, 9, 10));
-- ¿Cuales son los registros de productos (Products) cuyo ProductID sea un valor que esté en Shippers.ShipperID?
SELECT * FROM Products WHERE ProductID IN (SELECT ShipperID FROM Shippers);
